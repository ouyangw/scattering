# 1D Scattering Calculation Library (v1 MKL)
...
- The author does not have access to newer version of the MKL library which
  ...
  prohibitively high for large matrix.<sup>1</sup>
...
#### `LIBSCATTERING_MKL_INCLUDE`

The path to the headers of MKL. The default is `${MKLROOT}/include`. The
`${MKLROOT}` will be replaced by the environment variable.

#### `LIBSCATTERING_MKL_LIBS`

The MKL libraries' names to link. It must be a semicolumn(;) separated list.
The default is "`mkl_intel_lp64;mkl_core;mkl_sequential;pthread;m;dl`".

#### `LIBSCATTERING_MKL_LINKFLAG`

The link flag for the compiler to find the MKL library. The default is
"`-L${MKLROOT}/lib/intel64`". The `${MKLROOT}` will be replaced by the
...
The matrix A and vector B have complex numbers, so the value written will be in a
pair of real value and imaginary value, i.e. one number takes two columns. The
...
The variables are self-explanatory. The `energy` is the potential energy of the
reflection or transmission channel (state). The only subtlety is the `raw_norm`
and `normalized_norm`. The `raw_norm` is the plain absolute value of the
amplitude from the calculation result. The `normalized_norm` is the absolute
